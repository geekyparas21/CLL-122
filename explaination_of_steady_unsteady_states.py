...
class DesmosStyledPlotWithLabels(MovingCameraScene):
    def construct(self):
        ...
        # Draw each line from x = a to x = 480
        for a, color in zip(a_values, line_colors):
            ...
            line = axes.plot(
                line_func,
                color=color,
                x_range=[(a+600)/3, 60+((a+600)/3)]  # Start from 'a' to avoid plotting far left
            )
        ...
        # Intersection points (white dots)
        points = [
            # (313.37, 5.95),
            # (323.64, 45.4),
            (382.79, 7418.52),
        ...
        namesforpoints = [
            # Tex(r"$1$").next_to(axes.coords_to_point(*points[0]), UP*(0.05)+(0.05)*LEFT).scale(0.75),
            # Tex(r"$2$").next_to(axes.coords_to_point(*points[1]), UP*(0.05)+(0.05)*LEFT).scale(0.75),
            # Tex(r"$3$").next_to(axes.coords_to_point(*points[5]), UP*(0.05)+(0.05)*LEFT).scale(0.75),
            # Tex(r"$4$").next_to(axes.coords_to_point(*points[6]), UP*(0.05)+(0.05)*LEFT).scale(0.75),
            ...
            #Tex(r"$8$").next_to(axes.coords_to_point(*points[7]), DOWN).scale(0.75),
            #Tex(r"$9$").next_to(axes.coords_to_point(*points[8]), DOWN).scale(0.75),
        ]
        ...
        # Create colored boxes with labels for legend
